...
struct xHTTP_CLIENT {
  ...
  const char *pcEndOfData;
  char pcCurrentFilename[MAX_FILE_NAME_LENGTH];
  size_t uxBytesLeft;
  int Compressed;
  int State;
  YAROMFSFILE_HANDLE * pxFileHandle;
  char *pxCustomHandle;
  char *pxPostBuff;
  size_t CustomBytes;
  size_t MaxPostLength;
  int ClientInitialized;
  union {
    struct {
      uint32_t
          bReplySent : 1;
...
struct xFTP_CLIENT
{
	...
	YAROMFSFILE_HANDLE *pxReadHandle;
	YAROMFSFILE_HANDLE *pxWriteHandle;
	char pcCurrentDir[ SYS_FS_FILE_NAME_LEN ];
	char pcFileName[ SYS_FS_FILE_NAME_LEN ];
